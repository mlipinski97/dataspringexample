package pl.lipinski.springdataexample.util.validators;
...
public class UserRegistrationValidator implements Validator {
    ...
    public void validate(Object o, Errors errors) {
        ...
        ValidationUtils.rejectIfEmptyOrWhitespace(errors, "passwordConfirm", "passwordConfirm is empty");
        ...
        if(!user.getPassword().equals(user.getPasswordConfirm())){
            errorCode = 501;
            errors.reject("passowords does not match!");
            return;
        }
        if(userManager.findByUsername(user.getUsername()).isPresent()){
            errorCode = 502;
            errors.reject("This username is already registered!");
            return;
        }
